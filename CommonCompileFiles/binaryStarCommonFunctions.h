...
__device__ float4 calculatePlasmaPlasmaForce(float4 posMe, float4 posYou, float4 velMe, float4 velYou, float plasmaPushBackReduction)
{
	...
	else if(0.0f < r)
	{
		...
		// To get the force we need to multiply presure times area.
		// The area of contact is pi[(radius of an element)^2 - (seperation^2)/4)
		// In or units radius of a element is 1/2 so we get (pi/4)(1-seperation^2)
		// The presures of the elements are stored in vel.w.
		
		if(inOut <= 0) 	force_mag  = 1.0f - (velYou.w + velMe.w)*(PI*0.25f)*(1.0f - r2);
		else 			force_mag  = 1.0f - plasmaPushBackReduction*(velYou.w + velMe.w)*(PI*0.25f)*(1.0f - r2);
